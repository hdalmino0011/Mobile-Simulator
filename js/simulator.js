/**
 * js/simulator.js
 * Digital ATM â€“ Core Simulator Engine
 * 
 * Handles:
...
(function() {
    ...
    function log(message, type = 'info') {
        ...
        switch (type) {
            ...
            case 'warn':
                ...
                break;
            case 'error':
                console.error(`${prefix} âŒ ${message}`);
                break;
            case 'success':
    ...
    function buildStepHTML(step) {
        ...
        let html = `
            <div class="step-content active" data-step-id="${stepId}" data-step-number="${stepNumber}">
                <div class="step-indicator">
                    ${customerLabel} Â· Step ${stepNumber} of ${total}
                </div>
        `;

        // VIP badge
        if (isVip || step.isVip) {
            html += `<div class="vip-badge">â­ VIP Speedrun</div>`;
        }
        ...
        if (step.paragraphs && step.paragraphs.length > 0) {
            const nonBullets = step.paragraphs.filter(p => !p.startsWith('â€¢'));
        ...
        // Bullet lists
        if (step.paragraphs && step.paragraphs.length > 0) {
            const bullets = step.paragraphs.filter(p => p.startsWith('â€¢'));
            if (bullets.length > 0) {
                html += `<ul class="bullet-list">`;
                bullets.forEach(function(b) {
                    html += `<li>${b.replace('â€¢', '').trim()}</li>`;
                });
                html += `</ul>`;
            }
        }
    ...
    function buildVideoHTML(step) {
        return `
            <div class="video-container" data-action="play-video">
                ...
                <div class="video-label">ðŸŽ¬ HEY, I'M RYAN.</div>
            </div>
        `;
    }
    ...
    function buildExampleProductsHTML(step) {
        ...
        html += `<span class="label">ðŸ“¦ Pick one to start:</span>`;
    ...
    function buildLiveUrlHTML(step) {
        return `
            <div class="live-url-display">
                <div class="label">ðŸŒ Live at:</div>
    ...
    function buildSupportHTML(step) {
        return `
            <div class="support-callout">
                ${step.supportPhone ? `<div class="phone">ðŸ“ž ${step.supportPhone}</div>` : ''}
    ...
    function handlePrimary(e) {
        ...
        if (advanceTriggers.some(trigger => text.includes(trigger) || trigger.includes(text))) {
            if (state.currentStep < state.totalSteps - 1) {
                ...
                showToast('ðŸŽ‰ You\'ve completed all steps!');
            }
        } else {
            // Just show feedback for other buttons
            showToast(`ðŸ”˜ "${text}" clicked (simulated)`);
        }
    }
    ...
    function handleSecondary(e) {
        ...
        if (text.includes('Talk to someone') || text.includes('Do it for me')) {
            showToast(`ðŸ“ž "${text}" clicked (simulated)`);
        } else if (text === 'Show me others') {
            showToast('ðŸ“¦ Showing more product suggestions (simulated)');
        } else if (text === 'Just chat instead') {
            showToast('ðŸ’¬ Chat mode activated (simulated)');
        } else {
            showToast(`ðŸ”˜ "${text}" clicked (simulated)`);
        }
    }
    ...
    function handleExampleProduct(e) {
        ...
        showToast(`ðŸ“¦ Selected: "${text.substring(0, 40)}..."`);
    }

    function handleVideoPlay(e) {
        log('Video play button clicked', 'info');
        showToast('â–¶ï¸ Video playing (simulated)');
    ...
    function showHelp() {
        const helpMessage = `
ðŸ“± Digital ATM Simulator â€“ Keyboard Shortcuts

â† / â†‘  : Previous step
â†’ / â†“  : Next step
C      : Toggle customer type (Core â†” VIP)
...
?      : Show this help message
        ...
        showToast('âŒ¨ï¸ Help: Press ? for shortcuts');
        
        // Also log to console with better formatting
        console.log('%cðŸ“± Digital ATM Simulator Shortcuts', 'font-size:16px;font-weight:bold;');
        console.log('  â† / â†‘  : Previous step');
        console.log('  â†’ / â†“  : Next step');
    ...
    function init() {
        ...
        log('ðŸ’¡ Select a platform to begin', 'info');
        log('âŒ¨ï¸ Press "?" for keyboard shortcuts', 'info');
    }
    ...
    function bindBrowserControls() {
        ...
        if (DOM.chromeMenuBtn) {
            DOM.chromeMenuBtn.removeEventListener('click', function() {
                showToast('ðŸ“± Chrome menu opened (simulated)');
            });
        ...
        if (DOM.chromeMoreBtn) {
            DOM.chromeMoreBtn.removeEventListener('click', function() {
                showToast('ðŸ“± Chrome more options (simulated)');
            });
        ...
        if (safariMoreBtn) {
            safariMoreBtn.removeEventListener('click', function() {
                showToast('ðŸ“± Safari more options (simulated)');
            });
    ...
    window.Simulator = {
        ...
        // Debug
        debug: function() {
            console.log('%cðŸ“± Simulator Debug Info', 'font-size:14px;font-weight:bold;');
